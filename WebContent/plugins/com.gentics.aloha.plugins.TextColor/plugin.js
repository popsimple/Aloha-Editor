...
GENTICS.Aloha.TextColor.config = {
	colors : [ 'black', 'dimgray', 'gray', 'darkgray', 'lightgray', 'white', 'red', 'green', 'blue', 'yellowgreen', 'cadetblue', 'coral' ]
};
...
GENTICS.Aloha.TextColor.init = function() {
	...
	var that = this, buttons = {}, style = jQuery('<style></style>');
	...
	jQuery.each(GENTICS.Aloha.TextColor.config.colors, function(index, value) {
		GENTICS.Aloha.FloatingMenu.addButton("GENTICS.Aloha.continuoustext", buttons[value], 'Color',// that.i18n("floatingmenu.tab.color"),
		1);
	});
};